...
def process_account(nick):
    ...
    try:
        ...
        if not holdings:
            print(f"[{nick}] 총 트레이딩: {filtered_tot_evlu:,}원, 현금: {trading_cash:,}원, 시장비율: {int(mr):,}%({int(filtered_tot_evlu * int(mr) / 100):,}원), 현재비율: {current_ratio_v:.1f}%({total_eval:,}원) → 트레이딩 현금전환: {transfer_cash_need:,}원 전일저가 이탈 대상 없음 → 스킵")
            telegram_text = (f"✅ [{nick}] 총 트레이딩: {filtered_tot_evlu:,}원, 현금: {trading_cash:,}원\n" f"시장비율: {int(mr):,}%({int(filtered_tot_evlu * int(mr) / 100):,}원), 현재비율: {current_ratio_v:.1f}%({total_eval:,}원)\n" f" → 트레이딩 현금전환: {transfer_cash_need:,}원 전일저가 이탈 대상 없음")
        ...
        print(f"[{nick}] 총 트레이딩: {filtered_tot_evlu:,}원, 현금: {trading_cash:,}원, 시장비율: {int(mr):,}%({int(filtered_tot_evlu * int(mr) / 100):,}원), 현재비율: {current_ratio_v:.1f}%({total_eval:,}원) → 트레이딩 현금전환: {transfer_cash_need:,}원 매도대상: {len(orders)}건")
        ...
        if len(orders) > 0:
            ...
            summary_text = (f"📊 [{nick}] 총 트레이딩: {filtered_tot_evlu:,}원, 현금: {trading_cash:,}원\n" f"시장비율: {int(mr):,}%({int(filtered_tot_evlu * int(mr) / 100):,}원), 현재비율: {current_ratio_v:.1f}%({total_eval:,}원)\n" f" → 트레이딩 현금전환: {transfer_cash_need:,}원 전일저가 이탈 매도 대상 미존재")
            send_telegram(token, chat_id, summary_text)        
    except Exception as e:
